...
class MultiDimObsEncoder(ModuleAttrMixin):
    def __init__(
        self,
        shape_meta: dict,
        ...
        """
        Assumes rgb input: B,3,H,W
        Assumes depth input: B,1,H,W
    ...
    def forward(self, obs_dict):
        ...
        if self.share_depth_model:
            ...
            imgs = torch.cat(imgs, dim=0)
            # (N*B,D)
            feature = self.key_model_map["depth"](imgs)
            # (N,B,D)
            feature = feature.reshape(-1, batch_size, *feature.shape[1:])
            # (B,N,D)
            feature = torch.moveaxis(feature, 0, 1)
