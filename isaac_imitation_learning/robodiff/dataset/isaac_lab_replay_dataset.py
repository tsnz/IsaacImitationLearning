...
class IsaacLabReplayDataset(BaseImageDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        T_slice = slice(self.n_obs_steps)  # noqa: N806
...
def _convert_robomimic_to_replay(
    ...
    with h5py.File(dataset_path) as file:
        ...
        with tqdm(total=n_steps * len(rgb_keys), desc="Loading image data", mininterval=1.0) as pbar:  # noqa: SIM117
        ...
        with tqdm(total=n_steps * len(depth_keys), desc="Loading depth data", mininterval=1.0) as pbar:  # noqa: SIM117
            # one chunk per thread, therefore no synchronization needed
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
                futures = set()
                ...
                for f in completed:
                    if not f.result():
                        raise RuntimeError("Failed to encode depth information!")
                pbar.update(len(completed))
