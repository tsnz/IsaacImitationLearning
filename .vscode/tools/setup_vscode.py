...
IIL_DIR = pathlib.Path(__file__).parents[2]
...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    path_names.extend([f'"{ISAACLAB_DIR}/source/' + ext + '"' for ext in isaaclab_extensions])
...
def main():
    ...
    isaaclab_vscode_filename = os.path.join(IIL_DIR, ".vscode", "settings.json")
